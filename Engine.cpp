...
Engine::Engine() {
	...
	//Init light values
	this->light.pos_intensity = Vector4(0.0f, 1000.0f, 0.0f, 4000.0f);
...
void Engine::init(HWND* wHandle) {
	...
	//Create shaders
	this->shaders.init(this->device, this->deviceC);
	//this->createShaders();
...
void Engine::update(Camera* cam, std::vector<Geometry>* objects) {
	...
	//Update camera
	this->shaders.setCameraParams(cam->getProj(), cam->getView());
...
void Engine::updateMatrices(Camera* cam, Geometry* object) {
